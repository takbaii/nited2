...
(function(){
  ...
  const getGlobal=name=>{try{return Function('return (typeof '+name+'!=="undefined")?'+name+':null')();}catch(_){return null;}};
  const getAuth=()=>getGlobal('currentUser')?.authToken||'';
  const getConfig=()=>getGlobal('CONFIG')||window.CONFIG;
  const postForm=()=>getGlobal('postViaForm')||window.postViaForm;
  const apiGet=(action,data={})=>new Promise((resolve,reject)=>{
    const cfg=getConfig();
    if(!cfg?.SCRIPT_URL)return reject(new Error('ไม่พบ URL ของ Google Apps Script'));
    const cb='nited_teacher_'+Date.now()+'_'+Math.random().toString(36).slice(2),s=document.createElement('script');
    const clean=()=>{clearTimeout(timer);delete window[cb];s.remove();};let timer;
    window[cb]=r=>{clean();resolve(r||{});};
    s.onerror=()=>{clean();reject(new Error('เชื่อมต่อฐานข้อมูลไม่สำเร็จ'));};
    s.src=cfg.SCRIPT_URL+'?'+new URLSearchParams(Object.assign({action},data,{callback:cb})).toString();
    document.head.appendChild(s);timer=setTimeout(()=>{if(window[cb]){clean();reject(new Error('หมดเวลารอฐานข้อมูล'));}},20000);
  });
  const post=(action,data={})=>{const fn=postForm();if(!fn)return Promise.resolve({success:false,error:'ไม่พบตัวเชื่อมต่อฐานข้อมูล'});return fn(Object.assign({action,authToken:getAuth()},data));};
  async function load(){
    const host=document.getElementById('teacherDirectoryAdmin');if(!host)return;
    try{
      const r=await apiGet('getTeachers');const rows=r.data||[];
      host.innerHTML=`<div class="tda-head"><div><h3><i class="fas fa-users"></i> รายชื่อครู</h3><small>เพิ่ม แก้ไข ลบ และเปิด/ปิดการใช้งาน — บันทึกลง Google Sheets โดยตรง</small></div><button id="tdaAdd" class="btn btn-primary"><i class="fas fa-plus"></i> เพิ่มรายชื่อครู</button></div><div class="tda-tools"><input id="tdaSearch" placeholder="ค้นหารายชื่อครู..."><span>${rows.length} รายชื่อ</span></div><div class="tda-table-wrap"><table class="tda-table"><thead><tr><th>คำนำหน้า</th><th>ชื่อ</th><th>นามสกุล</th><th>ชื่อเต็ม</th><th>สถานะ</th><th>จัดการ</th></tr></thead><tbody id="tdaBody"></tbody></table></div>`;
      const render=(filter='')=>{const q=String(filter).toLowerCase();const list=rows.filter(x=>!q||[x.prefix,x.firstName,x.lastName,x.fullName].join(' ').toLowerCase().includes(q));const body=document.getElementById('tdaBody');body.innerHTML=list.length?list.map(x=>`<tr><td>${esc(x.prefix)}</td><td>${esc(x.firstName)}</td><td>${esc(x.lastName)}</td><td><strong>${esc(x.fullName)}</strong></td><td>${String(x.active).toLowerCase()==='false'?'<span class="acc-badge off">ปิด</span>':'<span class="acc-badge on">ใช้งาน</span>'}</td><td><button class="tda-edit" data-id="${esc(x.id)}"><i class="fas fa-pen"></i></button><button class="tda-del" data-id="${esc(x.id)}"><i class="fas fa-trash"></i></button></td></tr>`).join(''):'<tr><td colspan="6" class="acc-no-data">ไม่พบข้อมูล</td></tr>';};
      render('');document.getElementById('tdaSearch').oninput=e=>render(e.target.value);document.getElementById('tdaAdd').onclick=()=>editor(null,load);host.querySelectorAll('.tda-edit').forEach(b=>b.onclick=()=>editor(rows.find(x=>String(x.id)===String(b.dataset.id)),load));host.querySelectorAll('.tda-del').forEach(b=>b.onclick=async()=>{const row=rows.find(x=>String(x.id)===String(b.dataset.id));if(!confirm('ลบรายชื่อ '+(row?.fullName||'')+' จาก Google Sheets ใช่หรือไม่?'))return;const r=await post('deleteTeacher',{id:b.dataset.id});if(!r.success)return alert(r.error||'ลบไม่สำเร็จ');await load();refreshDirectory();});
    }catch(e){host.innerHTML=`<div class="acc-no-data">${esc(e.message)}</div>`;}
  }
  function editor(row,reload){
  ...
  function mount(){const cc=document.getElementById('adminControlCenter');if(!cc||document.getElementById('teacherDirectoryAdmin'))return;const section=document.createElement('section');section.id='teacherDirectoryAdmin';section.className='teacher-directory-admin';cc.appendChild(section);load();}
  document.addEventListener('DOMContentLoaded',()=>{setTimeout(mount,1000);const obs=new MutationObserver(()=>setTimeout(mount,100));const cc=document.getElementById('adminControlCenter');if(cc)obs.observe(cc,{childList:true,subtree:true});});
