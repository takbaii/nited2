...
(function () {
  ...
  /*
   * Navigation repair:
   * app.js changes the page title and .active class, but another script/CSS
   * can leave the dashboard visible. This wrapper makes the selected page
   * authoritative by setting display with !important after every navigation.
   */
  function installNavigationFix() {
    ...
    window.navigateTo = function (page) {
      ...
      try {
        ...
        originalError = err;
        console.error('[NITED navigation]', err);
      }
      ...
      const apply = function () {
        ...
        if (page === 'booking' && typeof window.loadMyBookings === 'function') {
          try { window.loadMyBookings(); } catch (e) { console.error(e); }
        }
        if (page === 'files' && typeof window.loadMyFiles === 'function') {
          try { window.loadMyFiles(); } catch (e) { console.error(e); }
        }
        if (page === 'evaluation' && typeof window.loadEvaluationData === 'function') {
          try { window.loadEvaluationData(); } catch (e) { console.error(e); }
        }
        if (page === 'admin' && typeof window.loadAdminData === 'function') {
          try { window.loadAdminData(); } catch (e) { console.error(e); }
        }
        if (page === 'dashboard' && typeof window.loadDashboard === 'function') {
          try { window.loadDashboard(); } catch (e) { console.error(e); }
        }
      };
  ...
  function startNavigationFix() {
    ...
    const timer = setInterval(function () {
      tries += 1;
      if (installNavigationFix() || tries > 50) clearInterval(timer);
    }, 100);
  }
